...
WAPI.waitNewMessages(false, async (data) => {
    for (let i = 0; i < data.length; i += 1) {
        console.log(data[i]);
        if (data[i].chatId.server.charAt(0) == "c" && data[i].type == "chat") {
            sendHttpRequest("POST", "http://localhost:3000/api/chat/response", { message: data[i].body, from: data[i].from })
                .then((data) => {
                    if (data.send&&!data.file) {
                        WAPI.sendMessage(data.to, data.message);
                    }else if(data.send&&data.file) {
                        WAPI.sendImage(data.image, data.to);
                        WAPI.sendMessage(data.to, data.message);
                    }
                })
        }
